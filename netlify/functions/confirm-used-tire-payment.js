...
exports.handler = async (event) => {
  ...
  if (session.metadata?.order_type !== 'used_tire') {
    return json(400, { message: 'This checkout session is not a used tire order.' });
  }
  ...
  if (!result.ok) {
    return json(result.statusCode || 500, { message: result.message || 'The paid order could not be confirmed.' });
  }
